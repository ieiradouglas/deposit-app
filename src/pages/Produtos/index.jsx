    ...
    return (
      <>
        <Modal isOpen={isOpen} onClose={onClose}>
          <ModalOverlay />
          <ModalContent maxWidth={'700px'} maxHeight={'500px'} padding={'30px 0px 0px 30px'} margin={'10px'} className="flex flex-col text-sm gap-2">
            <ModalCloseButton />
            <div className="flex absolute top-2 left-2 font-bold"><h3>ID:</h3><span>{prod.prod_id}</span></div>
            <div className="flex mt-2 items-start"><h3 className="text-sm font-bold">Nome:</h3><span>{prod.nome}</span></div>
            <div className="flex"><h3 className="text-sm font-bold">Descrição:</h3><span>{prod.descricao}</span></div>
            <div className="flex"><h3 className="text-sm font-bold">Valor:</h3><span>R$ {prod.preco}</span></div>
            <div className="flex"><h3 className="text-sm font-bold">Quantidade:</h3><span>{prod.estoque}</span></div>
            <div className="flex"><h3 className="text-sm font-bold">Categoria:</h3><span>{prod.categoria_id}</span></div>
            <div className="flex"><h3 className="text-sm font-bold">Fornecedor:</h3><span>{prod.fornecedor_id}</span></div>
            <div className="flex"><h3 className="text-sm font-bold">Data do Cadastro:</h3><span>{prod.data_adicao}</span></div>
            <ModalFooter>
              <Button colorScheme='red' mr={1} onClick={() => deleteProdutos(prod.produto_id)}>
                Excluir
              </Button>
            </ModalFooter>
          </ModalContent>
        ...
        <section className="flex flex-col">
          <div className="mb-3 flex flex-wrap gap-3">
            <NavLink to="/cadastro-produtos">
              <button onClick={() => console.log(...campoBuscaProduto)} className=" p-2 rounded-md cursor-pointer text-black bg-verde max-w-[140px]">Novo produto</button></NavLink>
            <div>
              <input onChange={(e) => { setCampoBuscaProduto(e.target.value) }}
                onKeyDown={() => getProdutos(campoBuscaProduto)}
                placeholder="Buscar produto..." className=" p-2 rounded-md border border-gray-950 text-black" />
            </div>
          </div>
